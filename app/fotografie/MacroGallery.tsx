...
const photos = [
  ...
  ["/images/makro/rotkehlchen.jpg", "Rotkehlchen auf moosigem Boden", "Rotkehlchen", "Das Rotkehlchen (Erithacus rubecula) ist einer der vertrautesten Vögel in unseren Gärten. Es sucht seine Nahrung meist am Boden und frisst vor allem Insekten, Larven, Spinnen und Würmer. Sein leiser, perlender Gesang ist häufig schon früh am Morgen zu hören."],
  ["/images/makro/hufeisen-azurjungfer.jpg", "Blaues Männchen einer Azurjungfer an einem Grashalm", "Hufeisen-Azurjungfer (wahrscheinlich)", "Wahrscheinlich ein Männchen der Hufeisen-Azurjungfer (Coenagrion puella). Die hellblaue Kleinlibelle gehört zu den häufigsten Arten an stehenden Gewässern. Das namensgebende schwarze Hufeisen auf dem zweiten Hinterleibssegment ist auf diesem Foto nicht sichtbar – deshalb bleibt die genaue Artbestimmung vorsichtig formuliert."],
  ["/images/makro/garten-sonnenauge.jpg", "Goldgelbe Blüten eines Garten-Sonnenauges", "Garten-Sonnenauge (wahrscheinlich)", "Wahrscheinlich ein Garten-Sonnenauge (Heliopsis helianthoides). Die ausdauernde Staude gehört zu den Korbblütlern und blüht mit ihren goldgelben Blüten meist von Sommer bis in den September. Das Sonnenauge ist eine beliebte Gartenpflanze und bietet Insekten reichlich Nahrung."],
  ["/images/makro/loewenzahn-pusteblume.jpg", "Pusteblume des Gewöhnlichen Löwenzahns", "Pusteblume", "Die verblühte Blüte des Gewöhnlichen Löwenzahns (Taraxacum officinale) wird zur Pusteblume. Jedes kleine Schirmchen trägt einen Samen, den der Wind – oder ein kräftiger Kinderpuster – auf die Reise schickt. Die feinen Flugschirme machen aus einer vertrauten Wiesenblume ein kleines Wunder der Verbreitung."],
] as const;
...
export function MacroGallery() {
  ...
  return <>
    ...
    <section className="macro-category"><h3>Libellen</h3><div className="gallery macro-gallery macro-previews" aria-label="Makroaufnahmen von Libellen">{image(photos[2], 2)}{image(photos[4], 4)}{image(photos[18], 18)}</div></section>
    ...
    <section className="macro-category"><h3>Vögel</h3><div className="gallery macro-gallery macro-previews" aria-label="Aufnahmen von Vögeln">{image(photos[17], 17)}</div></section>
    <section className="macro-category"><h3>Blumen</h3><div className="gallery macro-gallery macro-previews" aria-label="Aufnahmen von Blumen">{image(photos[19], 19)}{image(photos[20], 20)}</div></section>
